...
# ------------------- LOAD MODEL SAFELY -------------------
if not os.path.exists('model_forecast.pkl'):
    st.error("Model file not found. Please add model_forecast.pkl to the project folder.")
    st.stop()

if not os.path.exists('forecast_columns.pkl'):
    st.error("Columns file not found. Please add forecast_columns.pkl to the project folder.")
    st.stop()
...
if file is not None:
    data = pd.read_csv(file)  # Fixed: was pd.read_csv written as a hyperlink
